...
## フォーム更新（2026-09-08、最新）

**権限アプリレビューは未提出。** 本節が以下の過去記録より新しい。

- 審査担当者の英語操作手順を現在の審査専用環境へ更新し、自動保存を確認。専用ログイン・固定サブプロフィールだけ手動公開可能であること、通常ステージング利用者の公開停止、Autopilot・メールの停止を区別した。未完成の録画とUI/Cron検証を完成済みとは記載していない。
- アクセスコード欄、Webアプリのためストア用ギフトコード不要、地域による利用制限は実装していない旨を入力。プロフィールの制限と地域制限を区別した。
- 審査専用メールアドレスと専用パスワードをMetaへ渡すことをユーザーが明示許可。`meta-review-access.txt`（847 B）をMetaの審査担当者向け補足資料へ添付し、ファイル一覧と自動保存を確認。本文は秘密のためGitへ保存しない。アクセスコード欄からこの添付を案内した。
- Cloudflareの国一覧について、ユーザーは「想定は日本だけど任せる」と回答。利用者の所在国と処理国が異なることを説明。公開ネットワークとDeveloper Platformの再委託先から整理した134か国・地域を入力して保存した。再度編集画面を開き、日本・米国・リヒテンシュタイン・マルタ・ジンバブエの選択を再確認。追加81件はそれぞれ選択済み表示を確認している。根拠と一覧は `docs/meta-cloudflare-country-list.md`。実際に134か国へデータを送信したという実測記録ではない。
- threads_content_publish と threads_manage_replies の説明に残っていた「ステージングで全面投稿停止」を修正して保存。その他3権限の説明も読み直した。
- **この時点では5権限すべてが必須APIテスト0/1と表示**。Metaは最大24時間の反映待ちを案内。実際のAPI成功と、Metaのテストカウンターは区別する。以前の「basicにはカウンターなし」という記録は最新状態ではない。
- 全5権限とも動画未添付、許可用途への同意チェックは未完了。Google/Gemini・OpenRouterの申告は実際の利用範囲と契約条件の確定が必要であり、Cloudflareの入力完了だけで全委託先の申告完了と扱わない。
- 自動承認レビューによる認証情報添付と処理国一括入力の停止は、ユーザー回答を受けて解除され、対象操作は完了した。現在これらの承認を再要求する必要はない。
