...
- Metaの申請画面で `yama_skill` は「認証済み」。認証の進捗100%を確認。従来の「事業者認証の結果待ち」は解消した。Access Verificationの個別ステータスはこの画面だけでは判定していない。
...
公開・削除はまだ実施していない。投稿可能な環境を限定して検証してから、本人が確認した内容で実行する。
